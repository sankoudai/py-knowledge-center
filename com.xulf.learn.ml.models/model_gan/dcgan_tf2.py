...
def generator_loss(disc_logits):
    gen_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
        logits=disc_logits, labels=tf.ones([batch_size], dtype=tf.int32)))
    return gen_loss

def discriminator_loss(logits_fake, logits_real):
...
def run_optimization(real_images):
    ...
    with tf.GradientTape() as g:
        ...
        logits_fake = discriminator(fake_images, is_training=True)
        logit_real = discriminator(real_images, is_training=True)

        disc_loss = discriminator_loss(logits_fake, logit_real)
