...
class TestTensorArray(unittest.TestCase):
    ...
    def test_stack_unstack(self):
        ...
        ta2.unstack(t)
        assert_equal(ta.read(0), ta2.read(0))
        assert_equal(ta.read(1), ta2.read(1))
    ...
    def test_concat_split(self):
        ...
        assert_equal([1, 2, 3, 4], ta.concat())

        # split(tensor, lengths=None): 横向拆分，Tensor->TensorArray, rank保持
        ta = tf.TensorArray(tf.int32, size=2)
        ta.split([1, 2, 3, 4], lengths=[2, 3])
        assert_equal([1, 2], ta.read(0))
        assert_equal([3, 4], ta.read(0))
    ...
    def test_gather_scatter(self):
        ...
        ta = tf.TensorArray(tf.int32, size=3)
        ta.write(1, tf.constant([3, 4]))
        ta.scatter([0, 2], tf.constant([[1, 2],
                                        [5, 6]]))
        assert_equal([1, 2], ta.read(0))
        assert_equal([3, 4], ta.read(1))
        assert_equal([5, 6], ta.read(2))
